...
function getColortype(Type de point deau) {
          switch (Type de point deau) {
            case 'puits_ouvert':
              return  'blue';
            case 'point_equipe':
              return 'purple';
...
var getjsonEcoli = $.getJSON(urlpointdeau,function(data){
	...
	var typePE = L.geoJson(data,{
		...
		pointToLayer:function(feature,latlng){
			...
			var marker = L.circleMarker(latlng,{radius: 5,fillOpacity: 1, color: 'black',  fillColor: getColortype(feature.properties.Types de points deau),weight: 1,fillOpacity: 0.8,});
			//caractéristiques des popup
			marker.bindPopup('<b><u>Type de point d\'eau</u></b><br><i>' + feature.properties.Type de point deau
...
//transformation de la base de donné "point_eau.geojson" en une couche type de points d'eau
//lecture de la base de donnée
var getecolijson = $.getJSON(urlpointdeau,function(data3){
	//transformation de la base de donnée en couche (Layer)
	
	var Ecoli = L.geoJson(data3,{
		//Affichage sous forme de point
		pointToLayer:function(feature,latlng){
			//caractéristique des points
			var marker2 = L.circleMarker(latlng,{radius: 5,fillOpacity: 1, color: 'black', fillColor: getColorEcoli(feature.properties.D_E_Coli),weight: 1,fillOpacity: 0.8,});
			//caractéristiques des popup
			marker2.bindPopup('<b><u>Type de point d\'eau</u></b><br><i>' + feature.properties.Type de point deau
												+ ' <br><i>Comptage <em>E. coli</em>:</i> ' + feature.properties.D_E_Coli + ' UFC / 100 mL<br><b><u>Nom du point d\'eau : </u></b>'+feature.properties.A_nom);
			//Affichage des marqueurs
			return marker2;
		}
		});
	...
	controlLayers.addOverlay(Ecoli, "Contamination par E. coli" );
});


// création d’une couche geoJson qui appelle le fichier « localites.geojson »
var localites = $.getJSON(urlpointdeau,function(data2)
{
